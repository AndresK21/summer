...
class Cliente extends Validator{
	...
	public function setEstado2($value){
		if($this->validateId($value)){
			$this->estado2 = $value;
			return true;
		}else{
	...
	public function checkUsuario_cliente(){
		$sql = "SELECT id_cliente, nombres, apellidos FROM cliente INNER JOIN pedido USING(id_cliente) WHERE nombre_usuario = ?";
	...
	public function getVentas(){
		$sql = "SELECT pedido.fecha, total, pedido.id_pedido FROM pedido INNER JOIN cliente USING(id_cliente) WHERE pedido.estado = 1 AND id_cliente = ?";
		$params = array($this->id_cliente);
		return Database::getRows($sql, $params);
	}
	public function getVentas2($id){
		$sql = "SELECT detalle_pedido.cantidad, producto.nombre, precio, precio * detalle_pedido.cantidad AS subtotal, producto.id_producto FROM detalle_pedido INNER JOIN pedido USING(id_pedido) INNER JOIN cliente USING(id_cliente) INNER JOIN producto USING(id_producto) WHERE detalle_pedido.estado = 1 AND id_pedido = ?";
		$params = array($id);
		return Database::getRows($sql, $params);
	}
